...
def adapter_state_dict(model) -> dict:
    """Return only trainable adapter/granularity parameters from a model.state_dict()."""
    sd = model.state_dict()
    keep_keywords = ("adapter", "granularity_encoder", "gra_mem_scale")
    adapter_sd = {
        k: v.cpu()
        for k, v in sd.items()
        if any(keyword in k for keyword in keep_keywords)
    }
    if not adapter_sd:
        print("[warn] no adapter or granularity keys found when saving!")
    return adapter_sd
